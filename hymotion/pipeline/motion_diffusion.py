    ...
    def load_in_demo(
        ...
        if not allow_empty_ckpt:
            if not os.path.exists(ckpt_name):
                ...
                # Handle mean/std shape mismatch by loading them separately
                mean_key, std_key = "mean", "std"
                if mean_key in state_dict and std_key in state_dict:
                    # Update buffers with correct shape before load_state_dict
                    self.register_buffer("mean", state_dict[mean_key].clone())
                    self.register_buffer("std", state_dict[std_key].clone())

                self.load_state_dict(state_dict, strict=False)
        if mean_std_name is not None:
            assert os.path.exists(mean_std_name), f"{mean_std_name} not found"
            # Pass the path directly - _load_mean_std handles both files and directories
